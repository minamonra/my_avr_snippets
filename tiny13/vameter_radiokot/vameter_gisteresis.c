...
void DispIntShow (uint16_t Integer, uint8_t DispNum)
{
	...
	for (uint8_t i=0; i<4; i++){					//выводим цифры в цикле
...
ISR (TIM0_COMPA_vect)
{
	...
	if (! SimbOnMask){								//проверка на окончание цикла
	...
	for (uint8_t i=0; i<16; i++){								//цикл передачи 16 бит данных
	...
	if (++AdcCounter == 128){						//т.к. крутим 128 значений, то на каждый канал дается половина, т.е. 64
		AdcCounter = 0;
	...
	if (AdcCounter & 0x01){							//если TRUE то обработка ADC2, FALSE - обработка ADC3 (ну или наоборот :)...)
